...
name: wavread
version: 1.4.47
description: Operate WavRead and read its output correctly. Use whenever someone shares a WavRead export (.wavread.json, .wavread.md, .report.md), pastes a Track Document or Feedback Report, asks how to run an analysis or capture from their DAW, or asks for mix feedback derived from WavRead data. Covers reading order, evidence labelling, the app's known failure modes, the cross-table findings its rule engine cannot produce, comparison between versions, and the shape of a useful reply.
---

# WavRead

*Verified against WavRead 1.4.47 (analysis schema 2) on 2026-08-31. Where a user's build disagrees with this document, believe their build.*
...
1. **Provenance / Analysis Information first.** WavRead version, analysis schema,
   ...
   stages actually ran*, which decides what the rest of the document can mean.
...
## 2. Absence has two different meanings
...
## 3. What the mix was measured from
...
## 4. Label every claim with where it came from
...
## 5. Where a recommendation is allowed to come from
...
## 6. Recommend outcomes, not processors
...
## 7. Priority — what to raise first
...
## 8. Reading each section, and the trap in it
...
- **The mix measured above a stem table is the summed stems** — see §3.
...
## 9. Findings the rule engine cannot produce
...
- **Does the key label survive the chord table?** See §8, Overview. A modulation reads
...
## 10. Helping someone operate WavRead
...
- **Comparison between two takes is currently the reader's job.** WavRead does not
  yet emit a "since last take" diff. To compare, you need **both exports** — see §11.
...
**Check the document first.** If the user enabled it, the complete-analysis export
carries a **"Who this was written for"** section giving their mixing experience, the
kind of material they usually make, and the closest genre. It is the only part of the
document that describes a person rather than a recording, and it is there precisely so
you can pitch the reply at them. Use it before asking anything.

If it is absent, the user turned it off or exported a different document. Adapt the
depth, never the honesty.
...
| **Mastering engineer** | Delivery contract first, headroom, true peak, mono behaviour, LRA — and awareness that they will do the deciding |
| **Producer** | Arrangement, structure, density, part hierarchy |
| **Unknown** | Ask once, briefly. Then default to plain language with the numbers available underneath |

## 13. The songwriting boundary
...
## 14. The shape of your reply

Unless the user asked something narrower, answer in this order. Skip any part you
have no evidence for — an omitted section is correct; an invented one is not.
